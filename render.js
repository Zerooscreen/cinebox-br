...
const DEFAULT_TITLE = 'CineBox · Filmes e Séries em Alta: Sinopse, Avaliação, Elenco e Trailer Completo';
const DEFAULT_DESC = 'No CineBox você encontra sinopse, elenco, avaliação e trailer oficial dos filmes e séries mais populares do momento, incluindo informações de temporadas e episódios.';
const DEFAULT_OG_IMAGE = 'https://placehold.co/1200x630/17171b/8d8a92?text=CineBox';

// TODO: inserir o código de verificação do Google Search Console DESTE domínio
// (o código do cinebox-kr NÃO pode ser reaproveitado aqui -- ele está vinculado a outro domínio).
...
<footer>
  <p>CineBox — site de informações sobre filmes e séries com dados públicos do TMDB (não é um serviço de streaming) · Powered by <a href="https://www.themoviedb.org/" target="_blank" rel="noopener">TMDB</a></p>
  ${histatsSnippet()}
</footer>
...
function posterCard(item, type) {
  ...
  return `
    <a class="poster-card" href="/${type}/${item.id}/${encodeURIComponent(slug)}">
      ...
      <div class="poster-sub">${date || 'Ano desconhecido'}</div>
    </a>
  `;
}
...
function trailerBlock(videos) {
  ...
  if (!trailer) return `<div class="no-trailer">Nenhum trailer disponível.</div>`;
  return `
...
function castGrid(credits) {
  ...
  if (!cast.length) return `<div class="empty">Informações do elenco não disponíveis.</div>`;
